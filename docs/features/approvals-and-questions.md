...
- **Direct OpenAI Responses:** the current native executor exposes Read, Write, Edit and Bash. A question tool would need its own schema, the shared interaction broker, and an answer tool result associated with the original function call. SDK `PermissionResult` objects must not be sent as Responses tool output. This question-tool integration is not implemented by this change.
