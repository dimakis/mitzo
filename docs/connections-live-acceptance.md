...
Every failed identity test creates durable quarantine intent before retained sandboxes are stopped and detached. The last verified identity remains the baseline; test, rotation, and retry cannot reactivate the connection while quarantine cleanup is pending. Once detachment is confirmed, it remains `needs_attention` with a failed-test status rather than suggesting a recovered connection. Reconciliation retries that cleanup with its own deadline after a restart. Assignment completion updates the active consent state, appends its audit entry, and deletes the durable removal operation in one transaction. If a process stops after the active state was written by an older build, reconciliation safely clears the matching leftover operation instead of permanently blocking connection controls.
...
- `GH_TOKEN` (or `GITHUB_TOKEN`): controller-only GitHub write credential; it enables `github.publish-pr` but is never placed in a sandbox environment.
...
5. Force a failed identity test for a retained conversation. Confirm its provider is quarantined through stop/detach, the verified baseline identity is unchanged, and test, rotate, and retry remain blocked until any failed cleanup is reconciled after restart.
6. Remove an assignment and revoke a connection while a conversation is retained. Confirm shutdown, detachment, provider absence, and workspace preservation. Interrupt the controller during cleanup and confirm restart reconciliation completes it, including recovery of an already-active assignment with a leftover durable operation.
7. Verify browser/API errors, audit records, server logs, and saved metadata contain no submitted token. If upstream revocation is desired, revoke the token separately in Atlassian.
