...
The control displays the acknowledged effective mode, not an optimistic selection. Successful changes are saved and restored on switch/reconnect. If saving fails after the runtime accepts a change, clients still receive the effective mode and a separate persistence error. Requests to a running session serialize. Legacy WebSocket, v2 WebSocket, and REST mode changes use the same provider-aware transition. Existing-session sends restore server-owned mode rather than trusting a stale client selection. The picker is temporarily disabled while a new chat starts, and re-enabled on readiness or terminal failure. REST mode responses distinguish rejection from a mode that applied but could not be saved. Changing a mode does not undo an action already executing.
