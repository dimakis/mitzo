...
The template registry maps symbolic compiler, probe, and executor names to code-owned implementations through null-prototype, own-property lookup. Configuration cannot name filesystem paths or shell commands. Every implementation version also carries an explicit reviewed revision and a non-secret golden output fingerprint; a behavior change fails registry construction until that reviewed contract is intentionally updated (and a semantic template change creates a new template version). CI also hashes the full compiler and registry source artifacts, so a validation change outside a golden fixture fails the server build until reviewed artifacts are updated. Provider and capability declarations must reference each other bidirectionally.
...
IP publicness is fail-closed from checked-in, generated IANA data: IPv4 rejects every [IANA special-purpose range](https://www.iana.org/assignments/iana-ipv4-special-registry/), including globally reachable special assignments; IPv6 must both belong to IANA's [allocated global-unicast space](https://www.iana.org/assignments/ipv6-unicast-address-assignments/) and avoid every [IPv6 special-purpose range](https://www.iana.org/assignments/iana-ipv6-special-registry/). The runtime never fetches these registries. Snapshots retain source URLs and retrieval date; the server build verifies generated-table integrity and tests every table entry's lower, upper, and adjacent addresses. Updating requires a reviewed snapshot and generated-table refresh, so unallocated or newly special space fails closed until deliberately accepted.
