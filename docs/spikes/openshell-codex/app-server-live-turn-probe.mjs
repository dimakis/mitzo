...
server.on('exit', (code) => {
  if (!finished) done(1, `LIVE_APP_SERVER_TURN=server-exit:${code}`);
});
...
readline.createInterface({ input: server.stdout }).on('line', (line) => {
  ...
  if (message.error)
    return done(1, `LIVE_APP_SERVER_TURN=rpc-error:${message.error.message ?? 'unknown'}`);
    ...
    send({
      ...
      params: {
        ...
        input: [
          {
            type: 'text',
            text: 'Create /sandbox/workspaces/primary/live-turn.txt containing exactly LIVE_MODEL_TOOL_LOOP=pass. Use a shell command. Then reply with exactly done.',
          },
        ],
...
send({
  ...
  id: 0,
  params: {
    clientInfo: { name: 'mitzo_spike', version: '1.0.0' },
