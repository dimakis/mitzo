...
function names(root, relative = '') { return readdirSync(join(root, relative), { withFileTypes: true }).flatMap((entry) => entry.isDirectory() ? names(root, join(relative, entry.name)) : [join(relative, entry.name)]); }
function rpc(args, start, turn = false) {
  return new Promise((resolve, reject) => {
    const child = spawn(codex, ['app-server','--stdio',...args], { env, stdio: ['pipe','pipe','pipe'] }); let thread; let stderr=''; let done=false;
    ...
    readline.createInterface({input:child.stdout}).on('line',(line)=>{ let m; try {m=JSON.parse(line);} catch{return;} if(m.error)return finish(new Error(m.error.message));
      ...
      else if(m.id===1 && m.result?.thread?.id) { thread=m.result.thread.id; if (!turn) { clearTimeout(timeout); finish(null,m.result); } else child.stdin.write(JSON.stringify({method:'turn/start',id:2,params:{threadId:thread,input:[{type:'text',text:'Reply SYNTHETIC'}],approvalPolicy:'never',sandboxPolicy:{type:'externalSandbox',networkAccess:'restricted'}}})+'\n'); }
      else if(turn && m.method==='turn/completed' && m.params?.threadId===thread) { clearTimeout(timeout); finish(null,{thread:{id:thread},turn:m.params.turn}); }
    });
...
try {
  const fixture = http.createServer((request,response) => { request.resume(); response.writeHead(200,{'content-type':'application/json'}); response.end(JSON.stringify({id:'resp_local',object:'response',status:'completed',output:[{type:'message',role:'assistant',content:[{type:'output_text',text:'SYNTHETIC'}]}]})); });
  await new Promise((resolve) => fixture.listen(0,'127.0.0.1',resolve));
  const port = fixture.address().port;
  const config=['-c','model_provider="fixture"','-c',`model_providers.fixture={name="fixture",base_url="http://127.0.0.1:${port}/v1",env_key="FIXTURE_KEY",wire_api="responses"}`,'-c','model="fixture-model"']; env.FIXTURE_KEY='synthetic';
  const started = await rpc(config, () => ({method:'thread/start',id:1,params:{cwd:workspace}}), true); const thread = started.thread.id;
  await new Promise((resolve) => fixture.close(resolve));
  const providerEntries = existsSync(codexHome) ? readdirSync(codexHome).sort() : [];
  const sessionEntries = existsSync(join(codexHome, 'sessions')) ? names(join(codexHome, 'sessions')).sort() : [];
  ...
  const resumed = await rpc(config, () => ({method:'thread/resume',id:1,params:{threadId:thread}}));
  if(resumed.thread?.id!==thread || !JSON.stringify(resumed).includes('SYNTHETIC')) throw new Error('thread/resume did not retain synthetic history');
  console.log(`LOCAL_CODEX_CHECKPOINT_RESUME=pass provider_entries=${providerEntries.join(',')} session_entries=${sessionEntries.join(',')}`);
