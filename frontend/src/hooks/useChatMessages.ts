...
export function chatMessagesReducer(
  ...
  switch (action.type) {
    ...
    case 'RESTORE': {
      ...
      if (action.interrupted) {
        // Only preserve truly optimistic client-side sends (generated by
        // USER_SEND with `user-` prefix IDs) that aren't in the restored set.
        // This avoids re-introducing server-persisted user messages that may
        // have been intentionally omitted from a partial restore.
        ...
        // Merge optimistic messages into the restored timeline at their
        // original relative positions rather than blindly appending, so
        // conversation ordering is preserved.
        const merged: FinishedMessage[] = [...valid];
        for (const opt of optimisticUserMsgs) {
          // Find the original index in the local state to determine where
          // this message sat relative to restored messages.
          const localIdx = state.messages.indexOf(opt);
          // Find the last restored message that appeared before this
          // optimistic message in the local state.
          let insertAfter = -1;
