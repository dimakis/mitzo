  ...
  const invoke = vi.fn((_cli: string, args: string[]) =>
    args[0] === '--version'
      ...
      : JSON.stringify({
          ...
          version: '0.1.0',
          status: 'healthy',
          compute_drivers: [{ name: 'podman', capabilities: { driver_version: '0.1.0' } }],
        }),
  );
...
describe('Symposium production gate', () => {
  ...
  it('enables only explicit writer roles after live selected gateway and physical proofs', () => {
    ...
    expect(physical.verifyArtifactVolume).toHaveBeenCalledWith(
      'podman',
      'symposium-artifacts',
      'symposium',
    );
  ...
  it('fails closed without physical proof or on changed policy, gateway, or reviewer role', () => {
    const { config, attestation, physical, invoke } = setup();
    expect(() => verifySymposiumProductionGate(config, attestation, undefined, invoke)).toThrow(
      'proof is unavailable',
    );
    expect(() =>
      verifySymposiumProductionGate(
        config,
        { ...attestation, policySha256: sha('other') },
        physical,
        invoke,
      ),
    ...
    expect(() =>
      verifySymposiumProductionGate(
        config,
        { ...attestation, allowedRoles: ['reviewer'] } as never,
        physical,
        invoke,
      ),
    ).toThrow();
    expect(() =>
      verifySymposiumProductionGate(
        config,
        { ...attestation, allowedAccountProviders: ['anthropic-vertex'] } as never,
        physical,
        invoke,
      ),
    ).toThrow();
    ...
    expect(() =>
      verifySymposiumProductionGate(config, attestation, physical, wrongGateway),
    ).toThrow('gateway or compute driver changed');
  ...
  it('rejects ambiguous provider profile names before any physical probe', () => {
    ...
    expect(() => verifySymposiumProductionGate(config, duplicate, physical, invoke)).toThrow(
      'must be unique',
    );
