...
export function codexRuntimeOverrides(
  ...
  const parsed = z
    .object({
      ...
      // A user-selected Codex summary setting is safe to retain. Do not copy
      // arbitrary inherited configuration into the constrained runtime.
  ...
  const config: Record<string, unknown> = {
    ...
    // Summary deltas are the only Codex reasoning content that Mitzo exposes.
    // Ask for them by default, while preserving an explicit user preference.
